...
def publish_openmetadata_lineage(
    ...
    """Publish one Bronze -> Silver table-level lineage edge."""
    if not OPENMETADATA_ENABLED:
        return
    ...
    if not source_entity or not source_entity.get("id"):
        raise RuntimeError(
            f"Bronze OpenMetadata table was not found: {source_fqn}"
        )

    target_entity = get_openmetadata_table(target_fqn)
    if not target_entity or not target_entity.get("id"):
        raise RuntimeError(
            f"Silver OpenMetadata table was not found: {target_fqn}"
        )
    ...
    print(
        f"[{source_table}] OpenMetadata lineage published successfully: "
        f"{source_fqn} -> {target_fqn}"
    )
