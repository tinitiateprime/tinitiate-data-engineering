...
def validate_config_against_source(
    ...
    # Handler search ultimately calls the configured service search function.
    # Validate that every configured handler->service keyword is real.
    validate_parameter_list(
        service_path,
        cfg["service_search_function"],
        cfg.get("handler_service_parameters", []),
        "handler_service_parameters",
    )
    ...
    if cfg.get(
        ...
        validate_parameter_list(
            service_path,
            cfg["service_key_function"],
            cfg.get("handler_key_service_parameters", []),
            "handler_key_service_parameters",
        )
    ...
    # If handler schema names are configured, verify that the real handler
    # actually imports/defines those names. This prevents invented patch targets.
    handler_text = source_text(handler_path)
    for schema_field in (
        "handler_inner_schema",
        "handler_outer_schema",
        "handler_detail_outer_schema",
    ):
        schema_name = cfg.get(schema_field)
        if schema_name and schema_name not in handler_text:
            raise ValueError(
                f"{schema_field}='{schema_name}' does not exist in "
                f"{handler_path}"
            )
...
{execute_query_assertion(cfg, page_expr="10")}
...
def generate_one(
    ...
    *,
    ...
    # Defensive cleanup for handler tests:
    # direct patch replacements do not inject pytest/mock arguments.
    # Older generated variants could leave mock_metadata_model in
    # function signatures even though no such fixture exists.
