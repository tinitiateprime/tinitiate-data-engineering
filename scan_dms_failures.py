...
#  10. Support rolling-window PK merge, timestamp replace and legacy strategies.
...
#   - AUTO uses current-day processing when a timestamp exists; otherwise it performs an exact atomic full snapshot replacement.
#
###############################################################################
...
#   --CONTROL_SCHEMA
#
# OpenMetadata lineage parameters (optional):
#   --OPENMETADATA_ENABLED true|false
#   --OPENMETADATA_HOST http://openmetadata-host:8585
#   --OPENMETADATA_TOKEN <JWT token>
#   --OPENMETADATA_TOKEN_SECRET_ID <AWS Secrets Manager secret id/name>
...
#   --OPENMETADATA_PUBLISH_DEPENDENTS true|false
#
...
#   --SNAPSHOT_CLEAR_METHOD DELETE
#   --INCLUDE_TABLES table1,table2
#   --EXCLUDE_TABLES table3,table4
#
# AUTO strategy:
#   Timestamp + reliable key -> ROLLING_WINDOW_MERGE
#   Timestamp + no key       -> ROLLING_WINDOW_REPLACE
...
# OpenMetadata lineage publishing parameters.
# Lineage publishing is optional and never controls ETL success/failure.
OPENMETADATA_ENABLED = (
    get_runtime_parameter("OPENMETADATA_ENABLED", default="false")
    .strip()
    .lower()
    in {"1", "true", "t", "yes", "y"}
)

OPENMETADATA_HOST = get_runtime_parameter(
    "OPENMETADATA_HOST",
    default="",
).strip().rstrip("/")

# For a quick PoC, OPENMETADATA_TOKEN can be supplied as a Glue parameter.
# For production, prefer OPENMETADATA_TOKEN_SECRET_ID so the JWT is not
# stored directly in the Glue job arguments.
OPENMETADATA_TOKEN = get_runtime_parameter(
    "OPENMETADATA_TOKEN",
    default="",
).strip()

OPENMETADATA_TOKEN_SECRET_ID = get_runtime_parameter(
    "OPENMETADATA_TOKEN_SECRET_ID",
    default="",
).strip()

OPENMETADATA_BRONZE_SERVICE = get_runtime_parameter(
    "OPENMETADATA_BRONZE_SERVICE",
    default="NEXT Postgres Bronze",
).strip()

OPENMETADATA_SILVER_SERVICE = get_runtime_parameter(
    "OPENMETADATA_SILVER_SERVICE",
    default="NEXT Postgres Silver",
).strip()

OPENMETADATA_TIMEOUT_SECONDS = int(
...
if OPENMETADATA_TIMEOUT_SECONDS < 1:
    raise ValueError("OPENMETADATA_TIMEOUT_SECONDS must be at least 1.")

# When enabled, after publishing Bronze -> Silver lineage the framework also
# discovers PostgreSQL views/materialized views that directly depend on the
# successfully loaded Silver table and publishes Silver -> dependent-object
# lineage into OpenMetadata.
OPENMETADATA_PUBLISH_DEPENDENTS = (
    get_runtime_parameter(
        "OPENMETADATA_PUBLISH_DEPENDENTS",
        default="false",
    )
...
_OPENMETADATA_TOKEN_CACHE = None
...
LOOKBACK_DAYS = int(
    get_runtime_parameter("LOOKBACK_DAYS", default="2")
)
...
# DELETE is the FK-safe default. When the Silver foreign keys use
# ON DELETE CASCADE, deleting the parent automatically removes its children.
# TRUNCATE remains available only for schemas without foreign-key dependencies.
SNAPSHOT_CLEAR_METHOD = get_runtime_parameter(
    "SNAPSHOT_CLEAR_METHOD",
    default="DELETE",
).strip().upper()

if SNAPSHOT_CLEAR_METHOD not in {"DELETE", "TRUNCATE"}:
    raise ValueError(
        "SNAPSHOT_CLEAR_METHOD must be DELETE or TRUNCATE."
    )


def clear_target_for_snapshot(
    silver_conn,
    target_schema,
    target_table,
):
    """
    Clear a Silver target before a full snapshot reload.

    DELETE:
      - Works with foreign keys.
      - Executes ON DELETE CASCADE when configured on child constraints.
      - Preserves the existing table object, indexes, grants, and constraints.

    TRUNCATE:
      - Faster on independent tables.
      - Fails when another table references the target unless all related
        tables are truncated together or CASCADE is used.
    """
    with silver_conn.cursor() as cur:
        if SNAPSHOT_CLEAR_METHOD == "DELETE":
            cur.execute(
            ...
            print(
                f"[{target_table}] Snapshot clear used DELETE; "
                f"deleted_rows={affected_rows}."
            )
        ...
        print(
            f"[{target_table}] Snapshot clear used TRUNCATE."
        )
        return 0
...
def copy_rolling_window_bronze_to_temp(
    bronze_conn,
    silver_conn,
    source_table,
    temp_table,
    ...
    """
    Stream Bronze rows from the configured rolling timestamp window.

    With LOOKBACK_DAYS=2, the source predicate is:

        timestamp_column >= CURRENT_TIMESTAMP - INTERVAL '2 days'

    This deliberately rereads a small overlap. The keyed merge is idempotent:
    ...
    producer_thread = threading.Thread(
        target=producer,
        name=f"rolling-window-copy-{source_table}",
        daemon=True,
    )

    print(
        f"[{source_table}] Starting rolling-window COPY; "
    ...
    if consumer_error is not None:
        silver_conn.rollback()
        raise RuntimeError(
            f"Silver rolling-window COPY failed for {source_table}: "
            f"{consumer_error}"
        ) from consumer_error
    ...
    if producer_errors:
        ...
        producer_error = producer_errors[0]
        raise RuntimeError(
            f"Bronze rolling-window COPY failed for {source_table}: "
            f"{producer_error}"
        ) from producer_error

    print(
        f"[{source_table}] Rolling-window COPY completed in "
...
def rolling_window_replace_temp_to_target(
    silver_conn,
    temp_table,
    target_table,
    source_columns,
    ...
    Delete the same rolling timestamp slice from Silver and insert the Bronze
    slice loaded into TEMP. Keyed tables do not use this function; they use
    ROLLING_WINDOW_MERGE.
    """
...
def atomic_snapshot_replace_target_from_bronze(
    ...
    The existing Silver table is preserved:
      - Clear the target using DELETE by default.
      - ON DELETE CASCADE clears dependent child rows when configured.
...
def rolling_window_replace_target_from_bronze(
    ...
    try:
        ...
        with silver_conn.cursor() as silver_cur:
            if initial_load:
                # Close this cursor before using the shared clear helper.
                pass
            else:
                silver_cur.execute(
                    sql.SQL(
...
def full_refresh_temp_to_target(
    ...
    DELETE and INSERT run in one transaction by default. If INSERT fails,
    PostgreSQL rolls back the DELETE and any cascaded child deletes as well.
    """
    column_names = [column["name"] for column in source_columns]
...
###############################################################################
# OPENMETADATA BRONZE-TO-SILVER LINEAGE HELPERS
#
# The framework already knows the actual runtime source and target for every
# successfully loaded table. After final count validation succeeds, these
# helpers publish a table-level lineage edge into OpenMetadata:
#
#   NEXT Postgres Bronze.<BRONZE_DB>.<source_schema>.<source_table>
#                                ->
#   NEXT Postgres Silver.<SILVER_DB>.<target_schema>.<target_table>
#
# OpenMetadata publishing is intentionally non-blocking. Any OpenMetadata,
# network, authentication or lookup failure is logged as a WARNING and does
# not change the ETL table status.
###############################################################################


def get_openmetadata_api_base():
    """Return a normalized OpenMetadata /api/v1 base URL."""
    base = OPENMETADATA_HOST.rstrip("/")
...
def get_openmetadata_token():
    """
    Return the OpenMetadata JWT token.

    Priority:
      1. OPENMETADATA_TOKEN Glue/environment parameter.
      2. OPENMETADATA_TOKEN_SECRET_ID from AWS Secrets Manager.

    The Secrets Manager value may be either:
      - a raw JWT string, or
      - JSON containing one of: token, jwtToken, accessToken.
    """
    global _OPENMETADATA_TOKEN_CACHE

    if _OPENMETADATA_TOKEN_CACHE:
        return _OPENMETADATA_TOKEN_CACHE

    if OPENMETADATA_TOKEN:
        _OPENMETADATA_TOKEN_CACHE = OPENMETADATA_TOKEN
        return _OPENMETADATA_TOKEN_CACHE

    if not OPENMETADATA_TOKEN_SECRET_ID:
        raise RuntimeError(
            "OpenMetadata authentication is not configured. Supply either "
            "OPENMETADATA_TOKEN or OPENMETADATA_TOKEN_SECRET_ID."
        )

    secrets_client = boto3.client("secretsmanager")
    secret_response = secrets_client.get_secret_value(
        SecretId=OPENMETADATA_TOKEN_SECRET_ID
    )
    ...
    if not secret_value:
        raise RuntimeError(
            "OpenMetadata token secret does not contain SecretString."
        )

    token = secret_value.strip()

    # Support a JSON secret as well as a raw token secret.
    try:
        parsed = json.loads(token)
        if isinstance(parsed, dict):
            for key in ("token", "jwtToken", "accessToken"):
    ...
    if not token:
        raise RuntimeError("OpenMetadata token resolved to an empty value.")

    _OPENMETADATA_TOKEN_CACHE = token
    return _OPENMETADATA_TOKEN_CACHE


def openmetadata_request(method, path, payload=None):
    """Send one authenticated REST request to OpenMetadata."""
    if not OPENMETADATA_ENABLED:
        return None

    if not OPENMETADATA_HOST:
        raise RuntimeError(
            "OPENMETADATA_ENABLED=true but OPENMETADATA_HOST is empty."
        )

    url = get_openmetadata_api_base() + path
    body = None
    ...
    request = Request(
        ...
        method=method,
        headers={
            "Authorization": f"Bearer {get_openmetadata_token()}",
    ...
    try:
        with urlopen(
            request,
            timeout=OPENMETADATA_TIMEOUT_SECONDS,
        ) as response:
        ...
        raise RuntimeError(
            f"OpenMetadata HTTP {exc.code} for {method} {path}: "
            f"{error_body or exc.reason}"
        ) from exc

    except URLError as exc:
        raise RuntimeError(
            f"Unable to reach OpenMetadata at {url}: {exc}"
        ) from exc


def get_openmetadata_table(fqn):
    """Retrieve a table entity from OpenMetadata by fully qualified name."""
    encoded_fqn = quote(fqn, safe="")
    return openmetadata_request(
        "GET",
        f"/tables/name/{encoded_fqn}",
    )


def publish_openmetadata_edge(source_fqn, target_fqn, log_label):
    """Publish one table/view/materialized-view lineage edge."""
    source_entity = get_openmetadata_table(source_fqn)
    if not source_entity or not source_entity.get("id"):
        raise RuntimeError(
            f"OpenMetadata source entity was not found: {source_fqn}"
        )

    target_entity = get_openmetadata_table(target_fqn)
    if not target_entity or not target_entity.get("id"):
        raise RuntimeError(
            f"OpenMetadata target entity was not found: {target_fqn}"
        )
    ...
    print(
        f"[{log_label}] OpenMetadata lineage published successfully: "
        f"{source_fqn} -> {target_fqn}"
    )


def get_direct_dependent_views_and_materialized_views(
    silver_conn,
    schema_name,
    table_name,
):
    """
    Return PostgreSQL views and materialized views that directly depend on one
    Silver table.

    This intentionally uses PostgreSQL's dependency catalogs instead of trying
    to parse view SQL. It is the same relationship PostgreSQL uses internally
    to track table -> view/materialized-view dependencies.
    """
    with silver_conn.cursor() as cur:
        cur.execute(
            """
...
def publish_openmetadata_dependent_lineage(
    silver_conn,
    target_schema,
    target_table,
):
    """
    Publish Silver table -> dependent view/materialized-view edges.

    A missing dependent entity in OpenMetadata is logged and skipped so one
    catalog mismatch cannot stop the remaining lineage edges or the ETL job.
    """
    if not OPENMETADATA_PUBLISH_DEPENDENTS:
        return

    dependents = get_direct_dependent_views_and_materialized_views(
        silver_conn,
        target_schema,
        target_table,
    )
    ...
    print(
        f"[{target_table}] PostgreSQL dependent objects discovered: "
        f"{len(dependents)}"
    )
    ...
    for dependent in dependents:
        ...
        try:
            publish_openmetadata_edge(
                source_fqn=source_fqn,
                target_fqn=dependent_fqn,
                log_label=target_table,
            )
            published_count += 1
            ...
            skipped_count += 1
            print(
                f"[{target_table}] WARNING: OpenMetadata dependent lineage "
                f"publish failed for {dependent['type']} "
                f"{dependent['schema']}.{dependent['object']}: "
                f"{format_database_exception(dependent_error)}"
            )

    print(
        f"[{target_table}] OpenMetadata dependent lineage summary: "
        f"discovered={len(dependents)}, "
        f"published={published_count}, "
        f"skipped={skipped_count}."
    )


def publish_openmetadata_lineage(
    source_schema,
    source_table,
    target_schema,
    target_table,
    ...
    """
    Publish Bronze -> Silver table lineage and, when enabled, Silver ->
    dependent view/materialized-view lineage.
    """
    if not OPENMETADATA_ENABLED:
        return

    source_fqn = (
        f"{OPENMETADATA_BRONZE_SERVICE}."
        f"{BRONZE_DB}."
        f"{source_schema}."
        f"{source_table}"
    )
    ...
    print(
        f"[{source_table}] OpenMetadata lineage lookup: "
        f"{source_fqn} -> {target_fqn}"
    )
    ...
    publish_openmetadata_edge(
        ...
        log_label=source_table,
    )

    if OPENMETADATA_PUBLISH_DEPENDENTS:
        if silver_conn is None:
            raise RuntimeError(
                "OPENMETADATA_PUBLISH_DEPENDENTS=true but silver_conn "
                "was not supplied."
            )

        publish_openmetadata_dependent_lineage(
    ...
    try:
        ...
        if requested_load_strategy not in {
            ...
            raise RuntimeError(
                ...
                "ROLLING_WINDOW_MERGE, ROLLING_WINDOW_REPLACE, "
                "SYNC_WITH_FLAG, UPSERT, "
                "FULL_REFRESH, SNAPSHOT_REPLACE, APPEND. "
        ...
        if requested_load_strategy == "AUTO":
            if timestamp_column and primary_keys:
                load_strategy = "ROLLING_WINDOW_MERGE"
        ...
        elif requested_load_strategy == "CURRENT_DAY_MERGE":
            # Backward-compatible alias.
            load_strategy = "ROLLING_WINDOW_MERGE"

        elif requested_load_strategy == "CURRENT_DAY_REPLACE":
            # Backward-compatible alias.
            load_strategy = "ROLLING_WINDOW_REPLACE"
        ...
        if load_strategy == "SNAPSHOT_REPLACE":
            print(
                f"[{source_table}] No usable timestamp column. "
                f"Running exact full SNAPSHOT_REPLACE from Bronze."
            )
            ...
            # For a full snapshot, use exact counts even when general audit
            # counts are configured as estimated. This proves Bronze and Silver
            # match after the replacement.
            bronze_exact_count = get_row_count(
                bronze_conn,
                SOURCE_SCHEMA,
                source_table,
            )
            silver_exact_count = get_row_count(
                silver_conn,
                TARGET_SCHEMA,
                target_table,
            )

            bronze_count = bronze_exact_count
            silver_count = silver_exact_count
            rows_processed = silver_exact_count

            if bronze_exact_count != silver_exact_count:
                raise RuntimeError(
                    f"SNAPSHOT_REPLACE count mismatch for "
                    f"{SOURCE_SCHEMA}.{source_table}: "
                    f"Bronze={bronze_exact_count}, "
                    f"Silver={silver_exact_count}."
                )

            status = "SUCCESS"
            message = (
                "No timestamp column was available. Silver was atomically "
                "replaced with the complete Bronze table and exact counts "
                f"matched: {silver_exact_count} rows."
            )
            return
        ...
        if load_strategy in {
            ...
            #
            # Every later run reads the configured rolling timestamp window
            # and merges/replaces that slice in the populated Silver table.
            ...
            if silver_target_empty:
                ...
                message = (
                    ...
                    f"{load_strategy} using {timestamp_column} and "
                    f"LOOKBACK_DAYS={LOOKBACK_DAYS}."
                )
                return
            ...
            copy_rolling_window_bronze_to_temp(
                bronze_conn,
                silver_conn,
                source_table,
                temp_table,
            ...
            rolling_window_count = get_temp_row_count(
                silver_conn,
                temp_table,
            )

            print(
                f"[{source_table}] Rolling-window source rows="
                f"{rolling_window_count}; "
                f"lookback_days={LOOKBACK_DAYS}."
            )

            if load_strategy == "ROLLING_WINDOW_MERGE":
                ...
                message = (
                    f"Merged {rolling_window_count} rolling-window Bronze "
                    f"rows into Silver using {timestamp_column}, "
                    f"lookback_days={LOOKBACK_DAYS}, and key "
                    f"{primary_keys}."
                )

            else:
                replace_result = rolling_window_replace_temp_to_target(
                    silver_conn,
                    temp_table,
                    target_table,
                    source_columns,
                ...
                message = (
                    f"No reliable key was available. Replaced the "
                    f"{LOOKBACK_DAYS}-day Silver slice using "
                    f"{timestamp_column}; "
        ...
        #######################################################################
        # Publish successful Bronze -> Silver lineage to OpenMetadata.
        #
        # This is deliberately AFTER final exact-count validation so only a
        # table whose final ETL status is SUCCESS publishes lineage.
        #
        # OpenMetadata publishing is non-blocking. A lineage failure must never
        # turn a successful Bronze-to-Silver data load into an ETL failure.
        #######################################################################
        if status == "SUCCESS" and OPENMETADATA_ENABLED:
            try:
                publish_openmetadata_lineage(
                    source_schema=SOURCE_SCHEMA,
                    source_table=source_table,
                    target_schema=TARGET_SCHEMA,
                    target_table=target_table,
                ...
                print(
                    f"[{source_table}] WARNING: "
                    f"OpenMetadata lineage publish failed: "
                    f"{format_database_exception(lineage_error)}"
                )
...
def main():
    ...
    try:
        print("=" * 80)
        print("POSTGRES GLUE FRAMEWORK - VERSION 26 FK-SAFE DELETE SNAPSHOT")
        ...
        print(f"Snapshot clear     : {SNAPSHOT_CLEAR_METHOD}")
        print(f"OM lineage enabled : {OPENMETADATA_ENABLED}")
        if OPENMETADATA_ENABLED:
            print(f"OM host            : {OPENMETADATA_HOST or 'NOT CONFIGURED'}")
            print(f"OM Bronze service  : {OPENMETADATA_BRONZE_SERVICE}")
            print(f"OM Silver service  : {OPENMETADATA_SILVER_SERVICE}")
            print(
                f"OM dependents      : "
                f"{OPENMETADATA_PUBLISH_DEPENDENTS}"
            )
            print(
                "OM auth source     : "
                + (
                    "Glue/environment parameter"
                    if OPENMETADATA_TOKEN
                    else "Secrets Manager"
                    if OPENMETADATA_TOKEN_SECRET_ID
                    else "NOT CONFIGURED"
                )
            )
        ...
        print(
            f"Daily-load mode    : Populated Silver = "
            f"{LOOKBACK_DAYS}-day rolling merge"
        )
        print("No timestamp mode  : Exact atomic full snapshot every run")
        if SNAPSHOT_CLEAR_METHOD == "DELETE":
            print(
                "FK snapshot mode   : DELETE; child constraints must use "
                "ON DELETE CASCADE when parent deletes should remove children."
            )
